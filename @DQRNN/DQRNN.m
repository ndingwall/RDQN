classdef DQRNN
    ...
    properties (Access = public)
        bottleNeckState % Stores the current value of the bottleneck state (filterArch(end))
    end
    ...
    methods
        ...
        % Constructor
        function obj = DQRNN(inputSize,actions,filterArch, outputArch)
        ...
        function obj = set.w(obj,val)
            ...
            for i=2:size(obj.filterArch,2)
                obj.filterW{i} = reshape(val(index:(index - 1 + obj.filterArch(i) * (obj.filterArch(i-1) + 1))),...
                    [obj.filterArch(i), (obj.filterArch(i-1) + 1)]);
        ...
        function obj = useIdentity(obj)
            %% Use identity matrix for initialisation of recurrent weights
            bottleneckSize = obj.filterArch(end);
            obj.filterW{1}(:,1:bottleneckSize) = 0;
            obj.filterW{1}(1:bottleneckSize,1:bottleneckSize) = eye(bottleneckSize);
            if obj.filterArch(1) == obj.filterArch(2)
                obj.filterW{2} = [eye(obj.filterArch(1)), zeros(obj.filterArch(1),1)];
            end
        end
    end
end