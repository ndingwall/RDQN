function id = runCatchExperiment(minibatchSize, nInnerLoops, learningRate, dqnType, nHours, saveW, wStart)
...
nFuncEvals = 40000000; % number of function evaluations desired (300,000 takes approx 2 hours)
...
for expt = 1:numExperiments
    ...
    for session = 1:nSessions
        ...
        if (nNewTrans > 0) && (dqnType < 3)
            ...
            newTrans = [history, followCatchPolicy(dqn, wNew(:,end),epsilon, nNewTrans, gm, prob )];
            history = newTrans(max(1,(end - memoryLength + 1)) : end);
        end
        if (nNewTrans > 0) && (dqnType >= 3)
            %epsilon = max(epsilon - epsilonAnneal, minEpsilon);
            [~, newTrans] = followCatchPolicy(dqn, wNew(:,end), epsilon, nNewTrans, gm, prob);
...
%         end
        if mod(session, saveEvery) == 0
            ...
            fprintf('Win count is now %2.1f. SSE is %5.2f\n',winCount(expt,session), SSE(expt,session));
        end
