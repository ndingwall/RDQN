function id = runVarTManExperiment(minibatchSize, nInnerLoops, ...
...
nFuncEvals = 1e10; % number of function evaluations desired (300,000 takes approx 2 hours)
...
for expt = 1:numExperiments
    ...
    for session = 1:nSessions
        ...
        if (nNewTrans > 0) && (dqnType < 3)
            ...
            newTrans = [history, followVarTManPolicy(dqn, wNew(:,end), epsilon, ceil(nNewTrans / gm.L), gm, prob )];
            history = newTrans(max(1,(end - memoryLength + 1)) : end);
        end
        if (nNewTrans > 0) && (dqnType >= 3)
            %epsilon = max(epsilon - epsilonAnnealXXXXXXX, minEpsilon);
            [~, newGames] = followVarTManPolicy(dqn, wNew(:,end), epsilon, nNewTrans, gm, prob);
